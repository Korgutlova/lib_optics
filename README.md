...
<p>Библиотека предоставляет 2 класса линз: BiconvexLens(двояковыпуклая собирательная линза) и BiconcaveLens(двояковогнутая рассеивающая линза). Для создания объекта линзы нужно указать хотя бы 2 из трёх параметров: dist_subject(расстояние от линзы до объекта), dist_image(расстояние от линзы до изображения), focal_length(фокусное расстояние линзы). Так же необходимо указать height_subject(высоту предмета). Все остальные данные, а так же график строятся на основании этих входных данных.</p>
